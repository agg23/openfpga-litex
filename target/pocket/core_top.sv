...
module core_top (
    ...
    // GBA PHI clock (~1.058 MHz) – dedicated output registered in the I/O cell
    output wire cart_phi_clk,
  ...
  // -------------------------------------------------------------------------
  // 1.  Direction strobe two-flop synchronizers (one per bank/pin)
  // -------------------------------------------------------------------------

  logic dir_b0_meta1, dir_b0_sync;
  logic dir_b1_meta1, dir_b1_sync;
  logic dir_b2_meta1, dir_b2_sync;
  logic dir_b3_meta1, dir_b3_sync;
  logic dir_p31_meta1, dir_p31_sync;

  always_ff @(posedge clk_sys_57_12 or negedge reset_n) begin
    if (!reset_n) begin
      {dir_b0_meta1, dir_b0_sync,
       dir_b1_meta1, dir_b1_sync,
       dir_b2_meta1, dir_b2_sync,
       dir_b3_meta1, dir_b3_sync,
       dir_p31_meta1, dir_p31_sync} <= '0;
    end else begin
      // first stage: sample raw direction from the respective cores
      dir_b0_meta1  <= litex_cart_tran_bank0_dir;
      dir_b1_meta1  <= litex_cart_tran_bank1_dir;
      dir_b2_meta1  <= litex_cart_tran_bank2_dir;
      dir_b3_meta1  <= litex_cart_tran_bank3_dir;
      dir_p31_meta1 <= cart_tran_pin31_dir;   // raw – stays raw for output

      // second stage – provide de-glitched, in-clock versions
      dir_b0_sync   <= dir_b0_meta1;
      dir_b1_sync   <= dir_b1_meta1;
      dir_b2_sync   <= dir_b2_meta1;
      dir_b3_sync   <= dir_b3_meta1;
      dir_p31_sync  <= dir_p31_meta1;
    end
  end

  // -------------------------------------------------------------------------
  // 2. Data synchronizers – gated by the cleaned *_dir signals
  // -------------------------------------------------------------------------

  // *** Bank 0 (4-bit: cart_tran_bank0 comprises pins [7:4]) ***
  wire [3:0] cart_tran_bank0_sync;
  cart_pin_synchronizer #(.WIDTH(4)) bank0_sync (
      .clk          (clk_sys_57_12),
      .reset_n      (reset_n),
      .sample_enable({4{~dir_b0_sync}}),
      .raw_pins     (cart_tran_bank0),
      .sync_pins    (cart_tran_bank0_sync)
  );

  // *** Bank 1 ***
  wire [7:0] cart_tran_bank1_sync;
  cart_pin_synchronizer #(.WIDTH(8)) bank1_sync (
      .clk          (clk_sys_57_12),
      .reset_n      (reset_n),
      .sample_enable({8{~dir_b1_sync}}),
      .raw_pins     (cart_tran_bank1),
      .sync_pins    (cart_tran_bank1_sync)
  );

  // *** Bank 2 ***
  wire [7:0] cart_tran_bank2_sync;
  cart_pin_synchronizer #(.WIDTH(8)) bank2_sync (
      .clk          (clk_sys_57_12),
      .reset_n      (reset_n),
      .sample_enable({8{~dir_b2_sync}}),
      .raw_pins     (cart_tran_bank2),
      .sync_pins    (cart_tran_bank2_sync)
  );

  // *** Bank 3 ***
  wire [7:0] cart_tran_bank3_sync;
  cart_pin_synchronizer #(.WIDTH(8)) bank3_sync (
      .clk          (clk_sys_57_12),
      .reset_n      (reset_n),
      .sample_enable({8{~dir_b3_sync}}),
      .raw_pins     (cart_tran_bank3),
      .sync_pins    (cart_tran_bank3_sync)
  );

  // *** Pin 30 – WIDTH(1) is intentional & legal ***
  wire cart_tran_pin30_sync /* verilator public */;
  cart_pin_synchronizer #(.WIDTH(1)) pin30_sync (
      .clk          (clk_sys_57_12),
      .reset_n      (reset_n),
      // Pin 30 direction is generated by LiteX; treat that raw signal as stable enough
      .sample_enable(~cart_tran_pin30_dir),
      .raw_pins     (cart_tran_pin30),
      .sync_pins    (cart_tran_pin30_sync)
  );

  // *** Pin 31 – WIDTH(1) is intentional & legal ***
  wire cart_tran_pin31_sync;
  cart_pin_synchronizer #(.WIDTH(1)) pin31_sync (
      .clk          (clk_sys_57_12),
      .reset_n      (reset_n),
      .sample_enable(~dir_p31_sync),
      .raw_pins     (cart_tran_pin31),
      .sync_pins    (cart_tran_pin31_sync)
  );

  // UART select (output from Litex)
  wire use_cart_uart;

  // -------------------------------------------------------------------------
  // 3. Direction mux with break-before-make safety mechanism
  // -------------------------------------------------------------------------

  // Direction change detection and delayed propagation to prevent bus contention
  // Define constants for direction change delay
  localparam int DIR_CHANGE_DELAY_CYCLES = 7;  // 7-cycle counter value creates 8 cycles of Hi-Z at 57.12 MHz (≈140ns)
  localparam int DIR_COUNTER_WIDTH = 4;  // 4 bits needed for counter values 0-7
  
  // Individual direction signals  
  logic dir_b0_prev, dir_b1_prev, dir_b2_prev, dir_b3_prev, dir_p31_prev;
  logic [DIR_COUNTER_WIDTH-1:0] dir_change_delay_b0, dir_change_delay_b1, dir_change_delay_b2, 
                               dir_change_delay_b3, dir_change_delay_p31;
  logic dir_b0_safe, dir_b1_safe, dir_b2_safe, dir_b3_safe, dir_p31_safe;
  
  // Calculate effective direction values with UART override considered
  wire dir_b0_effective = use_cart_uart ? uart_cart_tran_bank0_dir : dir_b0_sync;
  wire dir_b3_effective = use_cart_uart ? uart_cart_tran_bank3_dir : dir_b3_sync;
  wire dir_p31_effective = use_cart_uart ? uart_cart_tran_pin31_dir : litex_cart_tran_pin31_dir;
  
  // Helper function to process direction changes with break-before-make logic
  function automatic logic update_safe_dir(
      input logic prev_dir,                        // Previous direction value
      input logic current_dir,                     // Current direction value
      input logic [DIR_COUNTER_WIDTH-1:0] counter  // Current delay counter
  );
    logic next_safe_dir;
    
    if (prev_dir != current_dir) begin
      // Direction changing - force to input during transition
      next_safe_dir = 1'b0;
    end else if (counter > 0 && counter == 4'h1) begin
      // Last delay cycle - apply new direction
      next_safe_dir = current_dir;
    end else if (counter > 0) begin
      // Still in delay period - maintain input mode
      next_safe_dir = 1'b0;
    end else begin
      // No change/transition complete - pass through value
      next_safe_dir = current_dir;
    end
    
    update_safe_dir = next_safe_dir; // Use function name assignment instead of return
  endfunction

  // Track previous direction values to detect changes
  always_ff @(posedge clk_sys_57_12 or negedge reset_n) begin
    if (!reset_n) begin
      dir_b0_prev <= 1'b0;
      dir_b1_prev <= 1'b0;
      ...
      dir_change_delay_b0 <= '0;
      dir_change_delay_b1 <= '0;
      dir_change_delay_b2 <= '0;
      dir_change_delay_b3 <= '0;
      dir_change_delay_p31 <= '0;
      ...
      // Store previous direction values
      dir_b0_prev <= dir_b0_effective;
      dir_b1_prev <= dir_b1_sync;
      dir_b2_prev <= dir_b2_sync;
      dir_b3_prev <= dir_b3_effective;
      dir_p31_prev <= dir_p31_effective;
      
      // Process all channels in a consistent way using the helper function
      
      // Bank 0 - Process direction change with break-before-make logic
      if (dir_b0_prev != dir_b0_effective) begin
        dir_change_delay_b0 <= DIR_CHANGE_DELAY_CYCLES;  // Start delay counter
        dir_b0_safe <= 1'b0;  // Force to input during transition
      end else if (dir_change_delay_b0 > 0) begin
        dir_change_delay_b0 <= dir_change_delay_b0 - 1'b1;  // Decrement counter
        // Update safe direction using the helper function
        dir_b0_safe <= update_safe_dir(dir_b0_prev, dir_b0_effective, 
                                       dir_change_delay_b0);
      end else begin
        dir_b0_safe <= dir_b0_effective;  // No transition - pass through
      end
      
      // Bank 1 - Process direction change
      if (dir_b1_prev != dir_b1_sync) begin
        dir_change_delay_b1 <= DIR_CHANGE_DELAY_CYCLES;
        dir_b1_safe <= 1'b0;
      end else if (dir_change_delay_b1 > 0) begin
        dir_change_delay_b1 <= dir_change_delay_b1 - 1'b1;
        dir_b1_safe <= update_safe_dir(dir_b1_prev, dir_b1_sync, 
                                       dir_change_delay_b1);
      end else begin
        dir_b1_safe <= dir_b1_sync;
      end
      
      // Bank 2 - Process direction change
      if (dir_b2_prev != dir_b2_sync) begin
        dir_change_delay_b2 <= DIR_CHANGE_DELAY_CYCLES;
        dir_b2_safe <= 1'b0;
      end else if (dir_change_delay_b2 > 0) begin
        dir_change_delay_b2 <= dir_change_delay_b2 - 1'b1;
        dir_b2_safe <= update_safe_dir(dir_b2_prev, dir_b2_sync, 
                                       dir_change_delay_b2);
      end else begin
        dir_b2_safe <= dir_b2_sync;
      end
      
      // Bank 3 - Process direction change
      if (dir_b3_prev != dir_b3_effective) begin
        dir_change_delay_b3 <= DIR_CHANGE_DELAY_CYCLES;
        dir_b3_safe <= 1'b0;
      end else if (dir_change_delay_b3 > 0) begin
        dir_change_delay_b3 <= dir_change_delay_b3 - 1'b1;
        dir_b3_safe <= update_safe_dir(dir_b3_prev, dir_b3_effective, 
                                       dir_change_delay_b3);
      end else begin
        dir_b3_safe <= dir_b3_effective;
      end
      
      // Pin 31 - Process direction change
      if (dir_p31_prev != dir_p31_effective) begin
        dir_change_delay_p31 <= DIR_CHANGE_DELAY_CYCLES;
        dir_p31_safe <= 1'b0;
      end else if (dir_change_delay_p31 > 0) begin
        dir_change_delay_p31 <= dir_change_delay_p31 - 1'b1;
        dir_p31_safe <= update_safe_dir(dir_p31_prev, dir_p31_effective, 
                                       dir_change_delay_p31);
      end else begin
        dir_p31_safe <= dir_p31_effective;
      end
    end
  end
  
  // Bank 0 & 3 support debug-key/UART override, others do not.
  // This is because Bank 0 carries the control signals (WR#, RD#, etc.) and Bank 3
  // carries the lower 8 bits of the address/data bus which are used for UART TX/RX.
  // Use the safe direction signals that implement break-before-make
  assign cart_tran_bank0_dir = dir_b0_safe;
  assign cart_tran_bank1_dir = dir_b1_safe;
  assign cart_tran_bank2_dir = dir_b2_safe;
  assign cart_tran_bank3_dir = dir_b3_safe;

  // Pin 30 direction is already driven by upstream LiteX logic – leave untouched.
  // Pin 31 uses break-before-make protection for IRQ/DRQ signals.
  assign cart_tran_pin31_dir = dir_p31_safe;

  // -------------------------------------------------------------------------
  // 4. Output bus tri-state control
  // -------------------------------------------------------------------------
  ...
  // Tri-state control for Bank-0 (4 bits).
  assign cart_tran_bank0 = cart_tran_bank0_dir ?
                          (use_cart_uart ? uart_cart_tran_bank0
                                        : litex_cart_tran_bank0)
                          : 4'hZ;

  // -------------------------------------------------------------------------
  // 5. Generate PHI clock (~1.058 MHz) – divide 57.12 MHz by 54 and
  //    register it in the I/O cell for minimal skew.
  // -------------------------------------------------------------------------
  // The Game Boy Advance / Game Boy cartridge interface uses a PHI (clock) signal at 
  // approximately 1 MHz. For optimal cartridge compatibility, we generate a clock that 
  // closely matches the original timing. 
  //
  // Design considerations:
  // - Target frequency: ~1.05 MHz (GBA PHI clock)
  // - Using 57.12 MHz base clock divided by 54 gives 1.058 MHz (-0.9% error)
  // - This small error is well within tolerance for most cartridges
  // - Using a power-of-two divider would give worse frequency accuracy
  // - The clock is registered directly at the I/O cell to minimize skew

  localparam int PHI_DIV = 54;  // 57.12 MHz / 54 ≈ 1.058 MHz (-0.9% error)

  logic [$clog2(PHI_DIV)-1:0] phi_cnt;  // Counter for clock division
  logic phi_i /* synthesis preserve */;  // Internal clock signal (preserved through synthesis)
  ...
  always_ff @(posedge clk_sys_57_12 or negedge reset_n) begin
    if (!reset_n) begin
      ...
      if (phi_cnt == PHI_DIV-1) begin
        phi_cnt <= '0;
        phi_i   <= ~phi_i;  // Toggle output to create a 50% duty cycle square wave
      end else begin
  ...
  // Fast output register hint for Quartus/Intel FPGAs ensures the clock is registered
  // directly in the I/O cell, minimizing clock path delay and skew
  (* altera_attribute = "-name FAST_OUTPUT_REGISTER ON" *)
  assign cart_phi_clk = phi_i;
  ...
  wire litex_cart_tran_bank1_dir;
  wire litex_cart_tran_bank2_dir;
  ...
  litex litex (
      ...
      .apf_cart_cart_bank0_in(cart_tran_bank0_sync),
      .apf_cart_cart_bank0_out(litex_cart_tran_bank0),
      .apf_cart_cart_bank0_dir(litex_cart_tran_bank0_dir),
      .apf_cart_cart_bank1_in(cart_tran_bank1_sync),
      .apf_cart_cart_bank1_out(litex_cart_tran_bank1),
      .apf_cart_cart_bank1_dir(litex_cart_tran_bank1_dir),
      .apf_cart_cart_bank2_in(cart_tran_bank2_sync),
      .apf_cart_cart_bank2_out(litex_cart_tran_bank2),
      .apf_cart_cart_bank2_dir(litex_cart_tran_bank2_dir),
      .apf_cart_cart_bank3_in(cart_tran_bank3_sync),
      .apf_cart_cart_bank3_out(litex_cart_tran_bank3),
      .apf_cart_cart_bank3_dir(litex_cart_tran_bank3_dir),
      .apf_cart_cart_pin30_in(cart_tran_pin30_sync),
      ...
      .apf_cart_cart_pin31_in(cart_tran_pin31_sync),
